...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        if (user == null || !passwordEncoder.matches(request.getPassword(), user.getPasswordHash())) {
            throw new BusinessException(HttpStatus.UNAUTHORIZED, "Username/email atau password tidak valid");
        }
    ...
    public void logout(String token) {
        if (activeSessions.remove(token) == null) {
            throw new BusinessException(HttpStatus.NOT_FOUND, "Session token tidak ditemukan");
        }
    }
}
