...
public String exportInboundDataToExcel(PabeanFilter filter) {
    try {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            createReportTemplate(workbook, sheet, "LAPORAN PEMASUKAN BARANG PER DOKUMEN PABEAN",
                    "Bukti Penerimaan Barang", "Pemasok/Pengirim", filter);
            ...
            fillDataRows(sheet, inboundList, inbound -> {
                ...
                setCellValueWithStyle(row, 3, formatDate(data.getDtCd()), centerStyle);
                setCellValueWithStyle(row, 4, data.getBillIo(), centerStyle);
                setCellValueWithStyle(row, 5, formatDate(data.getDtIo()), centerStyle);
...
public String exportOutboundDataToExcel(PabeanFilter filter) {
    try {
        ...
        if (filter != null) {
            log.info("Export outbound dengan filter: startDate={}, endDate={}, typeDoc={}", 
                filter.getStartDate(), filter.getEndDate(), filter.getTypeDoc());
        } else {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            createReportTemplate(workbook, sheet, "LAPORAN PENGELUARAN BARANG PER DOKUMEN PABEAN",
                    "Bukti Pengeluaran Barang", "Pembeli/Penerima", filter);
            CellStyle centerStyle = createDataStyle(workbook, false);
            CellStyle leftStyle = createDataStyle(workbook, true);
        ...
        log.info("Export outbound ke Excel sukses, size: {} bytes, total data: {}", excelBytes.length, outboundList.size());
        
        return base64Excel;
        
    } catch (IOException e) {
        log.error("Gagal export data outbound ke Excel", e);
        throw new RuntimeException("Gagal export data outbound ke Excel: " + e.getMessage());
    }
}

private void createReportTemplate(Workbook workbook, Sheet sheet, String reportTitle,
        String proofHeader, String partnerHeader, PabeanFilter filter) {
    ...
    titleCell.setCellValue("KAWASAN BERIKAT................................\n"
            + reportTitle + "\n"
            + "PERIODE: " + formatPeriode(filter != null ? filter.getStartDate() : null)
            + " S.D " + formatPeriode(filter != null ? filter.getEndDate() : null));
    titleCell.setCellStyle(createTitleStyle(workbook));

    Row headerRow = sheet.getRow(HEADER_ROW_INDEX);
    Row subHeaderRow = sheet.getRow(SUB_HEADER_ROW_INDEX);
    CellStyle headerStyle = createHeaderStyle(workbook, true, HorizontalAlignment.CENTER);

    sheet.addMergedRegion(new CellRangeAddress(HEADER_ROW_INDEX, SUB_HEADER_ROW_INDEX, 0, 0));
    sheet.addMergedRegion(new CellRangeAddress(HEADER_ROW_INDEX, SUB_HEADER_ROW_INDEX, 1, 1));
    sheet.addMergedRegion(new CellRangeAddress(HEADER_ROW_INDEX, HEADER_ROW_INDEX, 2, 3));
    sheet.addMergedRegion(new CellRangeAddress(HEADER_ROW_INDEX, HEADER_ROW_INDEX, 4, 5));
    sheet.addMergedRegion(new CellRangeAddress(HEADER_ROW_INDEX, SUB_HEADER_ROW_INDEX, 6, 6));
    sheet.addMergedRegion(new CellRangeAddress(HEADER_ROW_INDEX, SUB_HEADER_ROW_INDEX, 7, 7));
    sheet.addMergedRegion(new CellRangeAddress(HEADER_ROW_INDEX, SUB_HEADER_ROW_INDEX, 8, 8));
    sheet.addMergedRegion(new CellRangeAddress(HEADER_ROW_INDEX, SUB_HEADER_ROW_INDEX, 9, 9));
    sheet.addMergedRegion(new CellRangeAddress(HEADER_ROW_INDEX, SUB_HEADER_ROW_INDEX, 10, 10));
    sheet.addMergedRegion(new CellRangeAddress(HEADER_ROW_INDEX, SUB_HEADER_ROW_INDEX, 11, 11));

    String[] mainHeaders = {
            "No", "Jenis Dokumen", "Dokumen Pabean", "",
            proofHeader, "", partnerHeader, "Kode Barang", "Nama Barang", "Sat", "Jumlah", "Nilai Barang"
    };
    String[] subHeaders = {
...
private <T> void fillDataRows(Sheet sheet, List<T> dataList, Consumer<RowData<T>> rowFiller) {
...
private CellStyle createHeaderStyle(Workbook workbook, boolean bold, HorizontalAlignment alignment) {
    CellStyle style = workbook.createCellStyle();
    Font font = workbook.createFont();
    font.setBold(bold);
    style.setFont(font);
...
private void setCellValueWithStyle(Row row, int col, Object value, CellStyle style) {
...
private void setColumnWidths(Sheet sheet) {
    int[] widths = { 2816, 4817, 5817, 3818, 5817, 3818, 6817, 4817, 7817, 2816, 4817, 6817 };
